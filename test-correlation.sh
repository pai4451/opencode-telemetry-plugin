...
with open('/home/<username>/opencode/otel-data/traces.jsonl', 'r') as f:
    for line in f:
        if line.strip():
...
if [ -f "/home/<username>/opencode/otel-data/metrics.jsonl" ]; then
    echo "✓ Metrics file exists"
    echo -e "${YELLOW}Step 4: Checking for call_id in metrics...${NC}"
    if grep -q "call_id" /home/<username>/opencode/otel-data/metrics.jsonl; then
        echo "✓ call_id found in metrics"
    else
        echo "⚠ call_id not found in metrics (may need to use analyze_metrics.py first)"
    fi
else
    echo "⚠ Metrics file not found at /home/<username>/opencode/otel-data/metrics.jsonl"
